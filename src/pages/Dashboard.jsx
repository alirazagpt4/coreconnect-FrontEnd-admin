...
import {
    ...
    TrendingUp, Groups, Storefront, BarChart, FilterAlt , RestartAlt
...
const Dashboard = () => {
    ...
    // RESET FUNCTION
    const handleReset = () => {
        setRange('this_week'); // Wapas week par
        setCustomDates(initialCustomDates); // Dates clean
    };

    return (
        <Box sx={{ p: 3, bgcolor: '#f4f7f9', minHeight: '100vh' }}>

            {/* HEADER SECTION - Date restored */}
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
                <Box>
                    <Typography variant="h5" sx={{ fontWeight: 900, color: '#1a203e' }}>
                        Dashboard
                    </Typography>
            ...
            {/* CUSTOM FILTER BAR */}
            {range === 'custom' && (
                <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: '12px', border: '1px solid #ab1d4720' }}>
                    <Stack direction="row" spacing={2} justifyContent="flex-end" alignItems="center">
                        ...
                        <Button variant="contained" size="small" onClick={fetchStats} sx={{ bgcolor: '#ab1d47', fontWeight: 700 }}>Apply</Button>
                    </Stack>
                </Paper>
            )}

            {/* KPI ROW - 100% One Line Flex */}
            {loading ? (
...
const StatCard = ({ title, value, subtitle, icon, color }) => (
    <Paper elevation={0} sx={{
        ...
        <Typography sx={{ fontWeight: 900, color: '#1a203e', fontSize: '1.6rem', lineHeight: 1.1, mb: 1 }}>
            {value}
        </Typography>
